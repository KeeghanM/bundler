...
- `/product`
- README files
- requirements files, including `requirements.txt` and `requirements*.txt`
- `coding-standards.md`
- architecture notes
- ADRs
- issue templates
...
Commit messages must follow the convention defined in `coding-standards.md`.
...
Prefer commands documented in this order:

1. The closest applicable `AGENTS.md`
2. Package or service README
3. Root README
4. Existing package scripts or CI configuration
...
When tests cannot be run, explain why and provide the best available alternative verification.

Do not claim something is tested unless the relevant command has been run successfully.
...
Review the output before responding.
...
Do not omit Git status.

Do not omit test results.

Do not hide failed checks. Report them clearly with the likely cause and recommended next action.
...
- problem
- solution
- requirements or artefacts used
